...
export async function POST(req: NextRequest) {
  ...
  try {
    const rates = await abyssiniaProvider.fetchRates();
    ...
    return NextResponse.json({
      persisted: true,
      provider: abyssiniaProvider.slug,
      ...result,
      rates
    });
  } catch (error) {
    ...
    console.error("[ingest:abyssinia]", error);

    return NextResponse.json(
      {
        error: "Bank of Abyssinia ingestion failed",
        detail: message
      },
      { status: 502 }
    );
