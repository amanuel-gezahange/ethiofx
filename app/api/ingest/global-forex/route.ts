...
export async function POST(req: NextRequest) {
  ...
  try {
    const rates = await globalForexProvider.fetchRates();
    ...
    return NextResponse.json({
      persisted: true,
      provider: globalForexProvider.slug,
      ...result,
      rates
    });
  } catch (error) {
    ...
    console.error("[ingest:ammann]", error);

    return NextResponse.json(
      {
        error: "Ammann Forex Bureau ingestion failed",
        detail: message
      },
      { status: 502 }
    );
